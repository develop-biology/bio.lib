 ...
 * <br />
 * Functionally, what Vesicle does is implement the "protected" keyword by using chemical::Solvent mechanics. <br />
 ...
 * Furthermore, in a real-world solvent, such as water, some of the H2O molecules may decompose into H+ and OH-, forming small alkaline and acidic pockets which later combine back into neutral H2O. Small differences like this are natural and expected within a homogeneous solvent. We can thus quantify homogeneity as the inability to discern, probabilistically, the location of a significantly varied region, like a small cluster of H+ ions. Once we can say that a varied region is more likely to be in one place or another, we start to define a new solvent. For example, if all H2O molecules split and move to H+ on one side of a container and OH- on the other, we can say that we have one solvent of H+ and a separate solvent of OH-. This is another definition of Vesicle: a space which has a probability of containing some Solute which is different from the surrounding spaces. In our water example, the Vesicle has no hydrophobic encapsulation (as is typical in the real-world definition of "vesicle") and is instead simply an invisible line drawn between the H+ and OH-. This highlights the variability in Surfaces a Vesicle may have: completely transparent or completely opaque, both with nearly no Surface, or some highly specific opacity with a rich ecosystem of Surfaces. <br />
 * <br />
 * Based on the above description, there is no need to use a Solvent anywhere in your code. This is true. Don't use Solvents when you can use Vesicles.
 *
 */
class Vesicle :
	public Class< Vesicle >,
	virtual protected chemical::Solvent,
