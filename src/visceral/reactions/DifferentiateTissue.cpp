...
{
	Tissue* tissue = reactants[0];

	//If we were also given an environment. Otherwise, just ignore and move on.
	Tissue* environment;
	BIO_SANITIZE_WITH_CACHE(
		Cast<Tissue*>(reactants[1]),
		tissue->SetEnvironment(RESULT),)

	tissue->DifferentiateCells();
	return reactants;
}
